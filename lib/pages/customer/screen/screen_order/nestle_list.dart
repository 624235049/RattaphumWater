...
class NestleListOrder extends StatefulWidget {
  const NestleListOrder({Key? key}) : super(key: key);

  @override
  State<NestleListOrder> createState() => _NestleListOrderState();
}

class _NestleListOrderState extends State<NestleListOrder> {
  @override
  void initState() {
    super.initState();
    findlatlng();
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          image: DecorationImage(
                            image: NetworkImage(
                                '${API().BASE_URL}${productmodels[index]
                                    .waterImage}'),
                          ),
                        ),
                      ),
  ...
  Future<Null> readProductNestle() async {
    if (productmodels.length != 0) {
      productmodels.clear();
    }
    String url =
        '${API().BASE_URL}/rattaphumwater/getproduct_nestle.php?isAdd=true';
    await Dio().get(url).then((value) {
      setState(() {
        loadStatus = false;
      });
